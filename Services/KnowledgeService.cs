...
namespace PROJFACILITY.IA.Services
{
    public class KnowledgeService
    {
        ...
        // Configuração do índice (deve bater com o painel do Pinecone)
        ...
        public async Task ProcessarArquivoEIngerir(Stream arquivoStream, string nomeArquivo, string profissao)
        {
            try 
            {
                ...
                // Extração de texto conforme o tipo de arquivo
                ...
                {
                    await IngerirConhecimento(textoExtraido, profissao, nomeArquivo);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task IngerirConhecimento(string textoBruto, string profissao, string nomeArquivo)
        {
            // 1. Salva registro no SQL
            ...
            try 
            {
                // 2. Processa e envia para o Pinecone
            ...
            catch (Exception ex)
            {
                ...
                // Se falhar no Pinecone, remove do SQL para evitar dados órfãos
        ...
        // --- MÉTODO DE EXCLUSÃO CORRIGIDO (BLINDADO) ---
        public async Task<bool> ExcluirArquivo(int docId)
        {
            ...
            try
            {
                // Tenta apagar os vetores do Pinecone
                var index = _pinecone.Index(_indexName);
            ...
            {
                // Se der erro (ex: índice novo/vazio), apenas avisa no console e CONTINUA.
                // Isso garante que o arquivo saia da lista do Admin.
                Console.WriteLine($"[AVISO PINECONE]: Falha ao apagar vetores (Ignorado para limpar SQL). Erro: {ex.Message}");
            }

            // Remove do SQL (Obrigatório)
            _context.Documents.Remove(doc);
            await _context.SaveChangesAsync();
            return true;
        }
